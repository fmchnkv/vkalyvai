<? require($_SERVER["DOCUMENT_ROOT"] . "/template/header.php"); ?>
<?
$uri = $_SERVER['REQUEST_URI'];
?>
<div class="breadcrumbs">
    <div class="container">
        <a href="/pages/rezumes.php?client=Y<?= isset($auth) && $auth !== '' ? '&auth=' . urlencode($auth) : ''; ?>" class="breadcrumbs__back btn-link">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g>
                    <path d="M6.80762 2.80365C7.10055 2.51125 7.57543 2.51092 7.86816 2.80365C8.16073 3.0964 8.16052 3.57132 7.86816 3.8642L4.48145 7.25092H13.3379C13.7517 7.25127 14.0877 7.58707 14.0879 8.00092C14.0877 8.41477 13.7517 8.75057 13.3379 8.75092H4.48145L7.86816 12.1367C8.16084 12.4293 8.16041 12.9043 7.86816 13.1972C7.57527 13.4901 7.10051 13.4901 6.80762 13.1972L2.14062 8.53119C2.00005 8.39062 1.92098 8.19972 1.9209 8.00092C1.92098 7.8022 2.00015 7.61121 2.14062 7.47065L6.80762 2.80365Z" fill="#FC7827" />
                </g>
                <defs>
                    <clipPath>
                        <rect width="16" height="16" fill="white" />
                    </clipPath>
                </defs>
            </svg>
            <span>Назад в каталог</span>
        </a>
    </div>
</div>

<div class="detail">
    <div class="container">
        <div class="detail__row">
            <div class="detail__main">
                <div class="rezume">
                    <div class="rezume__header rezume-header detail__section">
                        <div class="rezume-header__top">
                            <div class="rezume-header__avatar avatar">
                                <img src="/images/avatar_rezume.png" alt="">
                            </div>
                            <div class="rezume-header__header offer-header">
                                <p class="offer-header__title">ОФИС-МЕНЕДЖЕР/СЕКРЕТАРЬ/помощник руководителя со знанием КДП и английского языка</p>
                                <ul class="offer-header__main">
                                    <li class="offer-header__main-item">Мужчина, 42 года</li>
                                    <li class="offer-header__main-item has-dot">Был сегодня</li>
                                    <li class="offer-header__main-item has-dot">Обновлено вчера</li>
                                </ul>
                                <span class="offer-header__sale">100 000 ₽</span>
                            </div>
                        </div>

                        <div class="rezume-header__bottom">
                            <ul class="rezume-header__tags tags">
                                <li class="tags__item tag tag_small tag_green">Активно ищу работу</li>
                                <li class="tags__item tag tag_small">Полная занятость</li>
                                <li class="tags__item tag tag_small">Удаленная работа</li>
                            </ul>

                            <ul class="rezume-header__info offer-info">
                                <li class="offer-info__item">
                                    <img src="/images/geo-icon.svg" alt="geo icon">
                                    <span>Город, район, метро</span>
                                </li>
                                <li class="offer-info__item has-dot">Не готов к командировкам</li>
                            </ul>

                            <ul class="rezume-header__profile profile profile--list">
                                <li class="profile__item">
                                    <span class="profile__label">Гражданство</span>
                                    <span class="profile__value">Гражданство РФ</span>
                                </li>
                                <li class="profile__item">
                                    <span class="profile__label">Участие в СВО</span>
                                    <span class="profile__value">Участник СВО</span>
                                </li>
                                <li class="profile__item">
                                    <span class="profile__label">Образование</span>
                                    <span class="profile__value">Высшее</span>
                                </li>
                                <li class="profile__item">
                                    <span class="profile__label">Общий опыт</span>
                                    <span class="profile__value">20 лет</span>
                                </li>
                            </ul>
                        </div>
                    </div>

                    <div class="rezume__about detail__section">
                        <span class="subcaption">Обо мне</span>
                        <div class="content">
                            <p>Давно выяснено, что при оценке дизайна и композиции читаемый текст мешает сосредоточиться. Lorem Ipsum используют потому, что тот обеспечивает более или менее стандартное заполнение шаблона, а также реальное распределение букв и пробелов в абзацах, которое не получается при простой дубликации "Здесь ваш текст.. Здесь ваш текст.. Здесь ваш текст.." Многие программы электронной вёрстки и редакторы HTML используют Lorem Ipsum в качестве текста по умолчанию, так что поиск по ключевым словам "lorem ipsum" сразу показывает, как много веб-страниц всё ещё дожидаются своего настоящего рождения. За прошедшие годы текст Lorem Ipsum получил много версий. Некоторые версии появились по ошибке, некоторые - намеренно (например, юмористические варианты).</p>
                        </div>
                    </div>

                    <div class="rezume__skills detail__section">
                        <span class="subcaption">Навыки</span>
                        <ul class="tags">
                            <li class="tags__item tag tag_size-m">Коммуникабельность</li>
                            <li class="tags__item tag tag_size-m">Выявление потребностей</li>
                            <li class="tags__item tag tag_size-m">Уравновешенность</li>
                            <li class="tags__item tag tag_size-m">Консультирование клиентов</li>
                            <li class="tags__item tag tag_size-m">Составление договоров</li>
                            <li class="tags__item tag tag_size-m">Пассивные продажи</li>
                            <li class="tags__item tag tag_size-m">Аудит компании</li>
                            <li class="tags__item tag tag_size-m">Анализ целевой аудитории</li>
                            <li class="tags__item tag tag_size-m">Наставничество</li>
                            <li class="tags__item tag tag_size-m">Планирование продаж</li>
                        </ul>
                    </div>

                    <div class="rezume__education rezume-education detail__section">
                        <span class="subcaption">Образование</span>

                        <ul class="rezume-education__list enum-list">
                            <li class="enum-list__item">
                                <div class="enum-list__nav enum-list__nav_big">
                                    <span class="enum-list__caption">Среднее специальное</span>
                                    <span class="enum-list__extra">2009</span>
                                </div>
                                <div class="enum-list__content">
                                    <span class="enum-list__title">Сибирский государственный университет телекоммуникаций и информатики</span>
                                    <span class="enum-list__subtitle">Фундаментальная информатика и информационные технологии</span>
                                </div>
                            </li>
                            <li class="enum-list__item">
                                <div class="enum-list__nav enum-list__nav_big">
                                    <span class="enum-list__caption">Высшее</span>
                                    <span class="enum-list__extra">2012</span>
                                </div>
                                <div class="enum-list__content">
                                    <span class="enum-list__title">Сибирский государственный университет телекоммуникаций и информатики</span>
                                    <span class="enum-list__subtitle">Фундаментальная информатика и информационные технологии</span>
                                </div>
                            </li>
                        </ul>
                    </div>

                    <div class="rezume__adv-trainings adv-trainings detail__section">
                        <span class="subcaption">Повышение квалификации</span>

                        <ul class="adv-trainings__list enum-list">
                            <li class="enum-list__item">
                                <div class="enum-list__nav">
                                    <span class="enum-list__extra">2009</span>
                                </div>
                                <div class="enum-list__content">
                                    <span class="enum-list__title">Skillbox</span>
                                    <span class="enum-list__subtitle">Специалист по информационной безопасности</span>
                                </div>
                            </li>
                            <li class="enum-list__item">
                                <div class="enum-list__nav">
                                    <span class="enum-list__extra">2009</span>
                                </div>
                                <div class="enum-list__content">
                                    <span class="enum-list__title">Skillbox</span>
                                    <span class="enum-list__subtitle">Специалист по информационной безопасности</span>
                                </div>
                            </li>
                            <li class="enum-list__item">
                                <div class="enum-list__nav">
                                    <span class="enum-list__extra">2009</span>
                                </div>
                                <div class="enum-list__content">
                                    <span class="enum-list__title">Skillbox</span>
                                    <span class="enum-list__subtitle">Специалист по информационной безопасности</span>
                                </div>
                            </li>
                        </ul>
                    </div>

                    <div class="rezume__experience rezume-experience detail__section">
                        <span class="subcaption">Опыт работы</span>

                        <ul class="rezume-experience__list enum-list">
                            <? for ($i = 0; $i < 4; $i++) : ?>
                                <li class="enum-list__item enum-list__item_big">
                                    <div class="enum-list__nav enum-list__nav_big">
                                        <span class="enum-list__caption">Сен. 2009 - Окт. 2013</span>
                                        <span class="enum-list__extra">4 года</span>
                                    </div>
                                    <div class="enum-list__content">
                                        <span class="enum-list__title">Название должности</span>
                                        <span class="enum-list__subtitle">Название компании</span>
                                        <div class="content">
                                            <span class="content__title content__title_small">Основные обязанности</span>
                                            <p>Давно выяснено, что при оценке дизайна и композиции читаемый текст мешает сосредоточиться. Lorem Ipsum используют потому, что тот обеспечивает более или менее стандартное заполнение шаблона, а также реальное распределение букв и пробелов в абзацах, которое не получается при простой дубликации "Здесь ваш текст.. Здесь ваш текст.. Здесь ваш текст.." Многие программы электронной вёрстки и редакторы HTML используют Lorem Ipsum в качестве текста по умолчанию, так что поиск по ключевым словам "lorem ipsum" сразу показывает, как много веб-страниц всё ещё дожидаются своего настоящего рождения. За прошедшие годы текст Lorem Ipsum получил много версий. Некоторые версии появились по ошибке, некоторые - намеренно (например, юмористические варианты).</p>
                                        </div>
                                    </div>
                                </li>
                            <? endfor; ?>
                        </ul>
                    </div>

                    <div class="rezume__driving-skills detail__section">
                        <span class="subcaption">Водительское удостоверение</span>
                        <ul class="tags">
                            <li class="tags__item tag tag_size-m">A</li>
                            <li class="tags__item tag tag_size-m">B</li>
                            <li class="tags__item tag tag_size-m">C</li>
                        </ul>
                    </div>

                    <div class="rezume__language-skills detail__section">
                        <span class="subcaption">Знание языков</span>
                        <ul class="tags">
                            <li class="tags__item tag tag_size-m">Русский</li>
                            <li class="tags__item tag tag_size-m">Английский</li>
                            <li class="tags__item tag tag_size-m">Казахский</li>
                        </ul>
                    </div>

                    <div class="rezume__portfolio portfolio-slider detail__section">
                        <span class="subcaption">Портфолио</span>
                        <div class="portfolio-slider__carousel swiper js-portfolio-slider">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide">
                                    <a class="portfolio-slider__preview-file js-preview-docx" href="#" data-doc="/pravki_04_08.docx">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <g>
                                                <path d="M19.2022 3.00098C19.6989 3.00098 20.1044 3.41035 20.1045 3.89355V20.1084C20.1042 20.3452 20.0093 20.5719 19.8418 20.7393C19.6743 20.9065 19.4477 21.0009 19.211 21.001H4.79787C4.67974 21.0001 4.56295 20.9757 4.45412 20.9297C4.34531 20.8837 4.24606 20.8175 4.16311 20.7334C4.08012 20.6492 4.01519 20.549 3.97072 20.4395C3.92627 20.33 3.90349 20.2129 3.90432 20.0947V8.40137L9.30666 3.00098H19.2022ZM10.2041 9.30078H5.70412V19.2012H18.3047V4.80078H10.2041V9.30078Z" fill="#FC7827" />
                                            </g>
                                        </svg>
                                        <span>pravki_04_08.docx</span>
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a class="portfolio-slider__preview-file js-preview-pdf" href="#" data-doc="https://vkalivay-new.blueberry-digital.ru/pravki_04_08.pdf">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <g>
                                                <path d="M19.2022 3.00098C19.6989 3.00098 20.1044 3.41035 20.1045 3.89355V20.1084C20.1042 20.3452 20.0093 20.5719 19.8418 20.7393C19.6743 20.9065 19.4477 21.0009 19.211 21.001H4.79787C4.67974 21.0001 4.56295 20.9757 4.45412 20.9297C4.34531 20.8837 4.24606 20.8175 4.16311 20.7334C4.08012 20.6492 4.01519 20.549 3.97072 20.4395C3.92627 20.33 3.90349 20.2129 3.90432 20.0947V8.40137L9.30666 3.00098H19.2022ZM10.2041 9.30078H5.70412V19.2012H18.3047V4.80078H10.2041V9.30078Z" fill="#FC7827" />
                                            </g>
                                        </svg>
                                        <span>pravki_04_08.pdf</span>
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/images/slide-1.png" data-fancybox="portfolio">
                                        <img src="/images/slide-1.png" alt="">
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/images/slide-2.png" data-fancybox="portfolio">
                                        <img src="/images/slide-2.png" alt="">
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/images/slide-3.png" data-fancybox="portfolio">
                                        <img src="/images/slide-3.png" alt="">
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/images/slide-1.png" data-fancybox="portfolio">
                                        <img src="/images/slide-1.png" alt="">
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/images/slide-2.png" data-fancybox="portfolio">
                                        <img src="/images/slide-2.png" alt="">
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/images/slide-3.png" data-fancybox="portfolio">
                                        <img src="/images/slide-3.png" alt="">
                                    </a>
                                </div>
                            </div>

                            <div class="slider-button slider-button_next icon-button">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g>
                                        <path d="M10.3066 3.6377C10.5995 3.34481 11.0743 3.34481 11.3672 3.6377L17.2002 9.47071C17.4931 9.7636 17.4931 10.2384 17.2002 10.5313L11.3672 16.3652C11.0743 16.6576 10.5994 16.6578 10.3066 16.3652C10.0139 16.0725 10.0142 15.5976 10.3066 15.3047L14.8594 10.751H3.33691C2.92296 10.7508 2.58709 10.4149 2.58691 10.001C2.58709 9.58703 2.92296 9.25116 3.33691 9.25098H14.8594L10.3066 4.69825C10.0139 4.40546 10.0141 3.93062 10.3066 3.6377Z" fill="#999999" />
                                    </g>
                                    <defs>
                                        <clipPath>
                                            <rect width="20" height="20" fill="white" />
                                        </clipPath>
                                    </defs>
                                </svg>
                            </div>
                            <div class="slider-button slider-button_prev icon-button">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g>
                                        <path d="M9.69336 3.6377C9.40047 3.34481 8.92571 3.34481 8.63281 3.6377L2.7998 9.47071C2.50691 9.7636 2.50691 10.2384 2.7998 10.5313L8.63281 16.3652C8.92569 16.6576 9.40062 16.6578 9.69336 16.3652C9.98609 16.0725 9.98576 15.5976 9.69336 15.3047L5.14062 10.751H16.6631C17.077 10.7508 17.4129 10.4149 17.4131 10.001C17.4129 9.58703 17.077 9.25116 16.6631 9.25098H5.14062L9.69336 4.69825C9.98614 4.40546 9.98593 3.93062 9.69336 3.6377Z" fill="#999999" />
                                    </g>
                                    <defs>
                                        <clipPath>
                                            <rect width="20" height="20" fill="white" transform="matrix(-1 0 0 1 20 0)" />
                                        </clipPath>
                                    </defs>
                                </svg>
                            </div>
                        </div>
                    </div>

                    <div class="rezume__sertificates sertificates-slider detail__section">
                        <span class="subcaption">Сертификаты и грамоты</span>

                        <div class="sertificates-slider__carousel swiper js-sertificates-slider">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide">
                                    <a href="/images/slide-s-1.png" data-fancybox="sertificates">
                                        <img src="/images/slide-s-1.png" alt="">
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/images/slide-s-2.png" data-fancybox="sertificates">
                                        <img src="/images/slide-s-2.png" alt="">
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/images/slide-s-1.png" data-fancybox="sertificates">
                                        <img src="/images/slide-s-1.png" alt="">
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/images/slide-s-2.png" data-fancybox="sertificates">
                                        <img src="/images/slide-s-2.png" alt="">
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/images/slide-s-1.png" data-fancybox="sertificates">
                                        <img src="/images/slide-s-1.png" alt="">
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/images/slide-s-2.png" data-fancybox="sertificates">
                                        <img src="/images/slide-s-2.png" alt="">
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/images/slide-s-1.png" data-fancybox="sertificates">
                                        <img src="/images/slide-s-1.png" alt="">
                                    </a>
                                </div>
                                <div class="swiper-slide">
                                    <a href="/images/slide-s-2.png" data-fancybox="sertificates">
                                        <img src="/images/slide-s-2.png" alt="">
                                    </a>
                                </div>
                            </div>

                            <div class="slider-button slider-button_next icon-button">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g>
                                        <path d="M10.3066 3.6377C10.5995 3.34481 11.0743 3.34481 11.3672 3.6377L17.2002 9.47071C17.4931 9.7636 17.4931 10.2384 17.2002 10.5313L11.3672 16.3652C11.0743 16.6576 10.5994 16.6578 10.3066 16.3652C10.0139 16.0725 10.0142 15.5976 10.3066 15.3047L14.8594 10.751H3.33691C2.92296 10.7508 2.58709 10.4149 2.58691 10.001C2.58709 9.58703 2.92296 9.25116 3.33691 9.25098H14.8594L10.3066 4.69825C10.0139 4.40546 10.0141 3.93062 10.3066 3.6377Z" fill="#999999" />
                                    </g>
                                    <defs>
                                        <clipPath>
                                            <rect width="20" height="20" fill="white" />
                                        </clipPath>
                                    </defs>
                                </svg>
                            </div>
                            <div class="slider-button slider-button_prev icon-button">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g>
                                        <path d="M9.69336 3.6377C9.40047 3.34481 8.92571 3.34481 8.63281 3.6377L2.7998 9.47071C2.50691 9.7636 2.50691 10.2384 2.7998 10.5313L8.63281 16.3652C8.92569 16.6576 9.40062 16.6578 9.69336 16.3652C9.98609 16.0725 9.98576 15.5976 9.69336 15.3047L5.14062 10.751H16.6631C17.077 10.7508 17.4129 10.4149 17.4131 10.001C17.4129 9.58703 17.077 9.25116 16.6631 9.25098H5.14062L9.69336 4.69825C9.98614 4.40546 9.98593 3.93062 9.69336 3.6377Z" fill="#999999" />
                                    </g>
                                    <defs>
                                        <clipPath>
                                            <rect width="20" height="20" fill="white" transform="matrix(-1 0 0 1 20 0)" />
                                        </clipPath>
                                    </defs>
                                </svg>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="detail__sidebar">
                <div class="detail__actions">
                    <a href="/chats/index.php?client=Y" class="detail__btn btn btn_primary btn_size-l">Связаться</a>
                    <button class="detail__like btn btn_light btn_size-m btn_has-icon"
                        <?= $auth && $auth == 'N' ? 'data-call-modal="registration_2"' : 'data-call-modal="favorite-comment"'; ?>>
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g>
                                <path d="M13.3789 3.25098C15.6573 3.25098 17.5038 5.12615 17.5039 7.75098C17.5039 13.0008 11.879 16.0009 10.0039 17.126C8.12876 16.0009 2.50391 13.0008 2.50391 7.75098C2.50406 5.12615 4.379 3.25098 6.62891 3.25098C8.02381 3.25098 9.25389 4.00103 10.0039 4.75098C10.7539 4.00103 11.984 3.25098 13.3789 3.25098ZM13.3789 4.75098C12.572 4.75098 11.6989 5.17861 11.0645 5.81152L10.0039 6.87207L8.94336 5.81152C8.30887 5.17861 7.43584 4.75098 6.62891 4.75098C5.174 4.75098 4.00405 5.99315 4.00391 7.75098C4.00391 9.95893 5.25314 11.9012 7.4873 13.6787C8.04605 14.1227 8.64296 14.5369 9.30371 14.9531C9.52795 15.0949 9.74967 15.2313 10.0039 15.3828C10.2581 15.2313 10.4799 15.0951 10.7041 14.9541C11.3649 14.5371 11.9615 14.1227 12.5195 13.6787C14.7553 11.9012 16.0039 9.95823 16.0039 7.75098C16.0038 5.98115 14.8511 4.75098 13.3789 4.75098Z" fill="#FC7827" />
                            </g>
                            <defs>
                                <clipPath>
                                    <rect width="20" height="20" fill="white" />
                                </clipPath>
                            </defs>
                        </svg>
                        <span>В избранное</span>
                    </button>
                    <button class="detail__share btn btn_light btn_size-m btn_has-icon js-share-btn" data-link="/pages/rezume.php?client=Y">
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g>
                                <path d="M12.6582 2.70603C13.3378 2.43957 14.0912 2.43052 14.7773 2.67966C15.4635 2.92894 16.0356 3.41992 16.3857 4.06052C16.7359 4.70125 16.8405 5.44791 16.6797 6.16013C16.5189 6.87225 16.1041 7.50159 15.5127 7.92966C14.9212 8.35769 14.1935 8.55509 13.4668 8.48533C12.7402 8.41551 12.0636 8.08339 11.5645 7.55076L8.41406 9.26755C8.53496 9.74857 8.53496 10.2524 8.41406 10.7334L11.5635 12.4511C12.0627 11.9183 12.739 11.5854 13.4658 11.5156C14.1926 11.4458 14.9202 11.6432 15.5117 12.0713C16.1032 12.4993 16.5178 13.1286 16.6787 13.8408C16.8396 14.553 16.7358 15.2997 16.3857 15.9404C16.0357 16.5811 15.4635 17.0719 14.7773 17.3213C14.091 17.5706 13.337 17.5615 12.6572 17.2949C11.9775 17.0283 11.4188 16.5224 11.085 15.873C10.7513 15.2238 10.6651 14.4753 10.8438 13.7676L7.69434 12.0508C7.28335 12.4899 6.74988 12.7955 6.16309 12.9277C5.5759 13.0599 4.96217 13.012 4.40234 12.791C3.84254 12.57 3.36197 12.1861 3.02344 11.6885C2.68489 11.1908 2.50391 10.6028 2.50391 10.001C2.50391 9.39908 2.68489 8.81108 3.02344 8.31345C3.36197 7.81584 3.84254 7.43193 4.40234 7.21091C4.96217 6.98989 5.5759 6.94204 6.16309 7.07419C6.74988 7.20636 7.28335 7.51196 7.69434 7.95115L10.8447 6.23435C10.6659 5.52647 10.7512 4.77728 11.085 4.1279C11.4188 3.47849 11.9784 2.97264 12.6582 2.70603ZM13.7539 13.001C13.3562 13.001 12.9746 13.1583 12.6934 13.4394C12.4121 13.7207 12.2539 14.1031 12.2539 14.501C12.254 14.8986 12.4121 15.2803 12.6934 15.5615C12.9746 15.8427 13.3562 16.0009 13.7539 16.001C14.1516 16.001 14.5332 15.8426 14.8145 15.5615C15.0957 15.2803 15.2538 14.8986 15.2539 14.501C15.2539 14.1031 15.0958 13.7207 14.8145 13.4394C14.5332 13.1584 14.1516 13.001 13.7539 13.001ZM5.50391 8.50095C5.10624 8.50099 4.72461 8.6583 4.44336 8.93943C4.16205 9.22073 4.00391 9.60313 4.00391 10.001C4.00401 10.3986 4.16214 10.7803 4.44336 11.0615C4.72462 11.3427 5.10619 11.5009 5.50391 11.501C5.90161 11.501 6.28317 11.3426 6.56445 11.0615C6.84567 10.7803 7.00381 10.3986 7.00391 10.001C7.00391 9.60313 6.84576 9.22073 6.56445 8.93943C6.28318 8.65836 5.90155 8.50095 5.50391 8.50095ZM13.7539 4.00095C13.3562 4.00099 12.9746 4.1583 12.6934 4.43943C12.4121 4.72073 12.2539 5.10313 12.2539 5.50095C12.254 5.89864 12.4121 6.28028 12.6934 6.5615C12.9746 6.8427 13.3562 7.00091 13.7539 7.00095C14.1516 7.00095 14.5332 6.84264 14.8145 6.5615C15.0957 6.28028 15.2538 5.89864 15.2539 5.50095C15.2539 5.10313 15.0958 4.72073 14.8145 4.43943C14.5332 4.15836 14.1516 4.00095 13.7539 4.00095Z" fill="#FC7827" />
                            </g>
                            <defs>
                                <clipPath>
                                    <rect width="20" height="20" fill="white" />
                                </clipPath>
                            </defs>
                        </svg>
                        <span>Поделиться</span>
                    </button>
                </div>
            </div>
        </div>
    </div>
</div>


<?
/*
<script type="module">
    // Импортируем главную библиотеку из CDN (используем версию mjs)
    import * as pdfjsLib from '/assets/js/pdf.mjs';

    // Настраиваем путь к воркеру (тоже mjs)
    pdfjsLib.GlobalWorkerOptions.workerSrc = '/assets/js/pdf.worker.mjs';

    async function renderPDF(url) {
        try {
            const container = document.querySelector("#docx-container");
            // Очищаем контейнер перед новым рендером
            container.replaceChildren();
            
            // Загружаем документ
            const loadingTask = pdfjsLib.getDocument({
                url: url
            });
            const pdf = await loadingTask.promise;
            console.log(`Документ загружен. Всего страниц: ${pdf.numPages}`);
            
            const scale = 0.9;

            // Цикл по всем страницам документа
            for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
                // 1. Получаем объект текущей страницы
                const page = await pdf.getPage(pageNum);
                const viewport = page.getViewport({ scale });

                // 2. Создаем уникальный элемент canvas для этой страницы
                const canvas = document.createElement("canvas");
                canvas.className = "pdf-page-canvas"; // Класс для стилизации (отступы, тени)
                canvas.style.display = "block";
                canvas.style.margin = "0 auto 20px auto"; // Центрирование и отступ снизу
                
                canvas.height = viewport.height;
                canvas.width = viewport.width;
                
                // Добавляем canvas в контейнер
                container.appendChild(canvas);

                // 3. Запускаем рендеринг страницы на созданный canvas
                const context = canvas.getContext('2d');
                const renderContext = {
                    canvasContext: context,
                    viewport: viewport
                };
                
                // Ждем завершения рендеринга текущей страницы перед переходом к следующей
                await page.render(renderContext).promise;
            }

            console.log('Все страницы успешно отрендерены через ESM');
            container.closest(".modal").classList.add("active");

        } catch (error) {
            console.error('Ошибка при работе с PDF.js:', error);
        }
    }

    document.addEventListener('DOMContentLoaded', () => {
        const pdfs = document.querySelectorAll('.js-preview-pdf');

        if (pdfs.length) {
            pdfs.forEach(pdf => {
                pdf.addEventListener('click', (e) => {
                    e.preventDefault();
                    renderPDF(pdf.dataset.file);
                })
            });
        }
    })
</script>
*/
?>



<? require($_SERVER["DOCUMENT_ROOT"] . "/template/footer.php"); ?>
