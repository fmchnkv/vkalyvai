<? require($_SERVER["DOCUMENT_ROOT"] . "/template/header.php"); ?>

<div class="breadcrumbs">
    <div class="container">
        <ul class="breadcrumbs__list">
            <li class="breadcrumbs__item breadcrumbs__item_home">
                <a href="/" class="link">Главная</a>
                <span>/</span>
            </li>
            <li class="breadcrumbs__item current">
                <span>Каталог вакансий</span>
            </li>
        </ul>
    </div>
</div>

<div class="catalog-nav">
    <div class="container">
        <div class="catalog-nav__inner">
            <div class="catalog-nav__actions">
                <button class="catalog-nav__nav-btn btn btn_outline btn_size-m btn_has-icon">
                    <span>Вакансии</span>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g>
                            <path d="M14.4735 7.38764C14.7664 7.09475 15.2411 7.09475 15.534 7.38764C15.8268 7.68055 15.8269 8.15535 15.534 8.44819L10.534 13.4482C10.2595 13.7226 9.82466 13.7401 9.53014 13.4999L9.4735 13.4482L4.4735 8.44819C4.18066 8.15535 4.18076 7.68055 4.4735 7.38764C4.76639 7.09475 5.24115 7.09475 5.53404 7.38764L10.0038 11.8574L14.4735 7.38764Z" fill="#FC7827" />
                        </g>
                    </svg>
                </button>

                <ul class="catalog-nav__tabs tabs">
                    <li class="tabs__item">
                        <a href="/pages/vacancies.php" class="tabs__link tab active">Вакансии</a>
                    </li>
                    <li class="tabs__item">
                        <a href="/pages/rezumes.php" class="tabs__link tab">Резюме</a>
                    </li>
                    <li class="tabs__item">
                        <a href="/pages/companies.php" class="tabs__link tab">Компании</a>
                    </li>
                </ul>

                <button class="catalog-nav__filter-btn btn btn_secondary btn_icon btn_icon-m js-filter-btn">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g url(#clip0_4580_14319)">
                            <path d="M5.50391 16.376H3.25391V14.126H5.50391V16.376ZM16.7539 16.001H7.00391V14.501H16.7539V16.001ZM5.50391 11.126H3.25391V8.87598H5.50391V11.126ZM16.7539 10.751H7.00391V9.25098H16.7539V10.751ZM5.50391 5.87598H3.25391V3.62598H5.50391V5.87598ZM16.7539 5.50098H7.00391V4.00098H16.7539V5.50098Z" fill="white" />
                        </g>
                        <defs>
                            <clipPath id="clip0_4580_14319">
                                <rect width="20" height="20" fill="white" />
                            </clipPath>
                        </defs>
                    </svg>
                </button>
            </div>

            <form class="catalog-nav__form inline-form">
                <div class="inline-form__row">
                    <div class="inline-form__input-wrapper">
                        <label class="inline-form__label field">
                            <svg class="field__icon" width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g >
                                    <path d="M11.1035 3.00098C15.5746 3.00098 19.2039 6.62957 19.2041 11.1006C19.2067 12.9384 18.5811 14.7223 17.4316 16.1562L21.2861 20.0098L20.0127 21.2832L16.1592 17.4287C14.7252 18.5782 12.9413 19.2038 11.1035 19.2012C6.6325 19.201 3.00391 15.5717 3.00391 11.1006C3.00412 6.6297 6.63263 3.00119 11.1035 3.00098ZM11.1035 4.80078C7.62263 4.80099 4.80392 7.6197 4.80371 11.1006C4.80371 14.5808 7.6225 17.4012 11.1035 17.4014C12.7419 17.4039 14.3166 16.7652 15.4912 15.623L15.626 15.4883C16.7682 14.3137 17.4069 12.739 17.4043 11.1006C17.4041 7.61957 14.5837 4.80078 11.1035 4.80078Z" fill="#FC7827" />
                                </g>
                                <defs>
                                    <clipPath id="clip0_4466_3472">
                                        <rect width="24" height="24" fill="white" />
                                    </clipPath>
                                </defs>
                            </svg>
                            <input class="field__input" type="text" name="search" placeholder="Какую работу вы ищете?">
                        </label>
                    </div>
                </div>
                <button class="inline-form__btn btn btn_primary btn_size-s">
                    <span>Найти работу</span>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g >
                            <path d="M9.25391 2.50098C12.9799 2.50098 16.0039 5.52498 16.0039 9.25098C16.0061 10.7825 15.4853 12.2689 14.5273 13.4639L17.7393 16.6748L16.6777 17.7363L13.4668 14.5244C12.2719 15.4823 10.7854 16.0032 9.25391 16.001C5.52791 16.001 2.50391 12.977 2.50391 9.25098C2.50391 5.52498 5.52791 2.50098 9.25391 2.50098ZM9.25391 4.00098C6.35291 4.00098 4.00391 6.34998 4.00391 9.25098C4.00391 12.1512 6.35291 14.501 9.25391 14.501C10.6192 14.5031 11.9313 13.9714 12.9102 13.0195L13.0225 12.9072C13.9743 11.9284 14.5061 10.6163 14.5039 9.25098C14.5039 6.34998 12.1542 4.00098 9.25391 4.00098Z" fill="white" />
                        </g>
                        <defs>
                            <clipPath id="clip0_4499_426">
                                <rect width="20" height="20" fill="white" />
                            </clipPath>
                        </defs>
                    </svg>
                </button>
            </form>

            <ul class="catalog-nav__filter-tags tags">
                <li class="tags__item tag tag_small tag_has-close">
                    <span>Примененный фильтр</span>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g url(#clip0_4740_26201)">
                            <path d="M13.1874 5.7575C13.4802 5.46501 13.9552 5.46489 14.248 5.7575C14.5407 6.05025 14.5405 6.52518 14.248 6.81805L11.7724 9.29364C11.382 9.68411 11.3821 10.3172 11.7724 10.7077L14.248 13.1833C14.5405 13.4762 14.5407 13.9511 14.248 14.2438C13.9552 14.5365 13.4803 14.5364 13.1874 14.2438L10.7118 11.7682C10.3213 11.3779 9.68823 11.3778 9.29776 11.7682L6.82315 14.2438C6.53042 14.5366 6.05548 14.5363 5.76261 14.2438C5.46976 13.951 5.46976 13.4761 5.76261 13.1833L8.23722 10.7077C8.62774 10.3172 8.62774 9.68416 8.23722 9.29364L5.76261 6.81805C5.46979 6.5252 5.46977 6.05034 5.76261 5.7575C6.05548 5.46507 6.53044 5.46482 6.82315 5.7575L9.29776 8.23309C9.68824 8.62357 10.3213 8.62348 10.7118 8.23309L13.1874 5.7575Z" fill="#999999" />
                        </g>
                        <defs>
                            <clipPath id="clip0_4740_26201">
                                <rect width="20" height="20" fill="white" />
                            </clipPath>
                        </defs>
                    </svg>
                </li>
                <li class="tags__item tag tag_small tag_has-close">
                    <span>Фильтр</span>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g url(#clip0_4740_26201)">
                            <path d="M13.1874 5.7575C13.4802 5.46501 13.9552 5.46489 14.248 5.7575C14.5407 6.05025 14.5405 6.52518 14.248 6.81805L11.7724 9.29364C11.382 9.68411 11.3821 10.3172 11.7724 10.7077L14.248 13.1833C14.5405 13.4762 14.5407 13.9511 14.248 14.2438C13.9552 14.5365 13.4803 14.5364 13.1874 14.2438L10.7118 11.7682C10.3213 11.3779 9.68823 11.3778 9.29776 11.7682L6.82315 14.2438C6.53042 14.5366 6.05548 14.5363 5.76261 14.2438C5.46976 13.951 5.46976 13.4761 5.76261 13.1833L8.23722 10.7077C8.62774 10.3172 8.62774 9.68416 8.23722 9.29364L5.76261 6.81805C5.46979 6.5252 5.46977 6.05034 5.76261 5.7575C6.05548 5.46507 6.53044 5.46482 6.82315 5.7575L9.29776 8.23309C9.68824 8.62357 10.3213 8.62348 10.7118 8.23309L13.1874 5.7575Z" fill="#999999" />
                        </g>
                        <defs>
                            <clipPath id="clip0_4740_26201">
                                <rect width="20" height="20" fill="white" />
                            </clipPath>
                        </defs>
                    </svg>
                </li>
                <li class="tags__item tag tag_small tag_has-close">
                    <span>Фильтр</span>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g url(#clip0_4740_26201)">
                            <path d="M13.1874 5.7575C13.4802 5.46501 13.9552 5.46489 14.248 5.7575C14.5407 6.05025 14.5405 6.52518 14.248 6.81805L11.7724 9.29364C11.382 9.68411 11.3821 10.3172 11.7724 10.7077L14.248 13.1833C14.5405 13.4762 14.5407 13.9511 14.248 14.2438C13.9552 14.5365 13.4803 14.5364 13.1874 14.2438L10.7118 11.7682C10.3213 11.3779 9.68823 11.3778 9.29776 11.7682L6.82315 14.2438C6.53042 14.5366 6.05548 14.5363 5.76261 14.2438C5.46976 13.951 5.46976 13.4761 5.76261 13.1833L8.23722 10.7077C8.62774 10.3172 8.62774 9.68416 8.23722 9.29364L5.76261 6.81805C5.46979 6.5252 5.46977 6.05034 5.76261 5.7575C6.05548 5.46507 6.53044 5.46482 6.82315 5.7575L9.29776 8.23309C9.68824 8.62357 10.3213 8.62348 10.7118 8.23309L13.1874 5.7575Z" fill="#999999" />
                        </g>
                        <defs>
                            <clipPath id="clip0_4740_26201">
                                <rect width="20" height="20" fill="white" />
                            </clipPath>
                        </defs>
                    </svg>
                </li>
                <li class="tags__item tag tag_small tag_has-close">
                    <span>Примененный фильтр</span>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g url(#clip0_4740_26201)">
                            <path d="M13.1874 5.7575C13.4802 5.46501 13.9552 5.46489 14.248 5.7575C14.5407 6.05025 14.5405 6.52518 14.248 6.81805L11.7724 9.29364C11.382 9.68411 11.3821 10.3172 11.7724 10.7077L14.248 13.1833C14.5405 13.4762 14.5407 13.9511 14.248 14.2438C13.9552 14.5365 13.4803 14.5364 13.1874 14.2438L10.7118 11.7682C10.3213 11.3779 9.68823 11.3778 9.29776 11.7682L6.82315 14.2438C6.53042 14.5366 6.05548 14.5363 5.76261 14.2438C5.46976 13.951 5.46976 13.4761 5.76261 13.1833L8.23722 10.7077C8.62774 10.3172 8.62774 9.68416 8.23722 9.29364L5.76261 6.81805C5.46979 6.5252 5.46977 6.05034 5.76261 5.7575C6.05548 5.46507 6.53044 5.46482 6.82315 5.7575L9.29776 8.23309C9.68824 8.62357 10.3213 8.62348 10.7118 8.23309L13.1874 5.7575Z" fill="#999999" />
                        </g>
                        <defs>
                            <clipPath id="clip0_4740_26201">
                                <rect width="20" height="20" fill="white" />
                            </clipPath>
                        </defs>
                    </svg>
                </li>
                <li class="tags__item tag tag_small tag_has-close">
                    <span>Примененный фильтр</span>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g url(#clip0_4740_26201)">
                            <path d="M13.1874 5.7575C13.4802 5.46501 13.9552 5.46489 14.248 5.7575C14.5407 6.05025 14.5405 6.52518 14.248 6.81805L11.7724 9.29364C11.382 9.68411 11.3821 10.3172 11.7724 10.7077L14.248 13.1833C14.5405 13.4762 14.5407 13.9511 14.248 14.2438C13.9552 14.5365 13.4803 14.5364 13.1874 14.2438L10.7118 11.7682C10.3213 11.3779 9.68823 11.3778 9.29776 11.7682L6.82315 14.2438C6.53042 14.5366 6.05548 14.5363 5.76261 14.2438C5.46976 13.951 5.46976 13.4761 5.76261 13.1833L8.23722 10.7077C8.62774 10.3172 8.62774 9.68416 8.23722 9.29364L5.76261 6.81805C5.46979 6.5252 5.46977 6.05034 5.76261 5.7575C6.05548 5.46507 6.53044 5.46482 6.82315 5.7575L9.29776 8.23309C9.68824 8.62357 10.3213 8.62348 10.7118 8.23309L13.1874 5.7575Z" fill="#999999" />
                        </g>
                        <defs>
                            <clipPath id="clip0_4740_26201">
                                <rect width="20" height="20" fill="white" />
                            </clipPath>
                        </defs>
                    </svg>
                </li>
                <li class="tags__item tag tag_small tag_has-close">
                    <span>Фильтр</span>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g url(#clip0_4740_26201)">
                            <path d="M13.1874 5.7575C13.4802 5.46501 13.9552 5.46489 14.248 5.7575C14.5407 6.05025 14.5405 6.52518 14.248 6.81805L11.7724 9.29364C11.382 9.68411 11.3821 10.3172 11.7724 10.7077L14.248 13.1833C14.5405 13.4762 14.5407 13.9511 14.248 14.2438C13.9552 14.5365 13.4803 14.5364 13.1874 14.2438L10.7118 11.7682C10.3213 11.3779 9.68823 11.3778 9.29776 11.7682L6.82315 14.2438C6.53042 14.5366 6.05548 14.5363 5.76261 14.2438C5.46976 13.951 5.46976 13.4761 5.76261 13.1833L8.23722 10.7077C8.62774 10.3172 8.62774 9.68416 8.23722 9.29364L5.76261 6.81805C5.46979 6.5252 5.46977 6.05034 5.76261 5.7575C6.05548 5.46507 6.53044 5.46482 6.82315 5.7575L9.29776 8.23309C9.68824 8.62357 10.3213 8.62348 10.7118 8.23309L13.1874 5.7575Z" fill="#999999" />
                        </g>
                        <defs>
                            <clipPath id="clip0_4740_26201">
                                <rect width="20" height="20" fill="white" />
                            </clipPath>
                        </defs>
                    </svg>
                </li>
                <li class="tags__item tag tag_small tag_has-close">
                    <span>Фильтр</span>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g url(#clip0_4740_26201)">
                            <path d="M13.1874 5.7575C13.4802 5.46501 13.9552 5.46489 14.248 5.7575C14.5407 6.05025 14.5405 6.52518 14.248 6.81805L11.7724 9.29364C11.382 9.68411 11.3821 10.3172 11.7724 10.7077L14.248 13.1833C14.5405 13.4762 14.5407 13.9511 14.248 14.2438C13.9552 14.5365 13.4803 14.5364 13.1874 14.2438L10.7118 11.7682C10.3213 11.3779 9.68823 11.3778 9.29776 11.7682L6.82315 14.2438C6.53042 14.5366 6.05548 14.5363 5.76261 14.2438C5.46976 13.951 5.46976 13.4761 5.76261 13.1833L8.23722 10.7077C8.62774 10.3172 8.62774 9.68416 8.23722 9.29364L5.76261 6.81805C5.46979 6.5252 5.46977 6.05034 5.76261 5.7575C6.05548 5.46507 6.53044 5.46482 6.82315 5.7575L9.29776 8.23309C9.68824 8.62357 10.3213 8.62348 10.7118 8.23309L13.1874 5.7575Z" fill="#999999" />
                        </g>
                        <defs>
                            <clipPath id="clip0_4740_26201">
                                <rect width="20" height="20" fill="white" />
                            </clipPath>
                        </defs>
                    </svg>
                </li>
                <li class="tags__item tag tag_small tag_has-close">
                    <span>Примененный фильтр</span>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g url(#clip0_4740_26201)">
                            <path d="M13.1874 5.7575C13.4802 5.46501 13.9552 5.46489 14.248 5.7575C14.5407 6.05025 14.5405 6.52518 14.248 6.81805L11.7724 9.29364C11.382 9.68411 11.3821 10.3172 11.7724 10.7077L14.248 13.1833C14.5405 13.4762 14.5407 13.9511 14.248 14.2438C13.9552 14.5365 13.4803 14.5364 13.1874 14.2438L10.7118 11.7682C10.3213 11.3779 9.68823 11.3778 9.29776 11.7682L6.82315 14.2438C6.53042 14.5366 6.05548 14.5363 5.76261 14.2438C5.46976 13.951 5.46976 13.4761 5.76261 13.1833L8.23722 10.7077C8.62774 10.3172 8.62774 9.68416 8.23722 9.29364L5.76261 6.81805C5.46979 6.5252 5.46977 6.05034 5.76261 5.7575C6.05548 5.46507 6.53044 5.46482 6.82315 5.7575L9.29776 8.23309C9.68824 8.62357 10.3213 8.62348 10.7118 8.23309L13.1874 5.7575Z" fill="#999999" />
                        </g>
                        <defs>
                            <clipPath id="clip0_4740_26201">
                                <rect width="20" height="20" fill="white" />
                            </clipPath>
                        </defs>
                    </svg>
                </li>
            </ul>
        </div>
    </div>
</div>

<section class="catalog catalog--vacancies">
    <div class="container">
        <div class="catalog__top catalog-header">
            <button class="catalog-header__filter-btn btn btn_secondary btn_has-icon js-filter-btn">
                <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g url(#clip0_4580_14319)">
                        <path d="M5.50391 16.376H3.25391V14.126H5.50391V16.376ZM16.7539 16.001H7.00391V14.501H16.7539V16.001ZM5.50391 11.126H3.25391V8.87598H5.50391V11.126ZM16.7539 10.751H7.00391V9.25098H16.7539V10.751ZM5.50391 5.87598H3.25391V3.62598H5.50391V5.87598ZM16.7539 5.50098H7.00391V4.00098H16.7539V5.50098Z" fill="white" />
                    </g>
                    <defs>
                        <clipPath id="clip0_4580_14319">
                            <rect width="20" height="20" fill="white" />
                        </clipPath>
                    </defs>
                </svg>
                <span>Показать фильтры</span>
            </button>

            <div class="catalog-header__row">
                <span class="catalog-header__counter">Найдено 16 177 вакансий</span>

                <div class="catalog-header__sorting sorting">
                    <div class="sorting__item">
                        <button class="sorting__button accordion-button btn-link js-sort-btn">
                            <span>Сначала самые свежие</span>
                            <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g >
                                    <path d="M10.0009 2.58301C10.4149 2.58318 10.7507 2.91905 10.7509 3.33301V14.8555L15.3046 10.3027C15.5976 10.0103 16.0724 10.01 16.3652 10.3027C16.6577 10.5955 16.6575 11.0704 16.3652 11.3633L10.5312 17.1963C10.2383 17.4892 9.76354 17.4892 9.47065 17.1963L3.63764 11.3633C3.34475 11.0704 3.34475 10.5956 3.63764 10.3027C3.93056 10.0102 4.4054 10.0099 4.69819 10.3027L9.25092 14.8555V3.33301C9.2511 2.91905 9.58696 2.58318 10.0009 2.58301Z" fill="#FC7827" />
                                </g>
                                <defs>
                                    <clipPath id="clip0_4580_8402">
                                        <rect width="16" height="16" fill="white" />
                                    </clipPath>
                                </defs>
                            </svg>
                        </button>
                        <!-- TODO: add dropdown  -->
                        <div class="sorting__dropdown" data-id="sort">
                            <ul class="sorting__dropdown-list">
                                <li class="sorting__option active" data-value="Сначала самые свежие">Сначала самые свежие</li>
                                <li class="sorting__option" data-value="По размеру зарплаты">По размеру зарплаты</li>
                                <li class="sorting__option" data-value="По удаленности от вас">По удаленности от вас</li>
                            </ul>
                        </div>
                    </div>
                    <div class="sorting__item">
                        <button class="sorting__button accordion-button btn-link js-sort-btn">
                            <span>За все время</span>
                            <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g >
                                    <path d="M10.0009 2.58301C10.4149 2.58318 10.7507 2.91905 10.7509 3.33301V14.8555L15.3046 10.3027C15.5976 10.0103 16.0724 10.01 16.3652 10.3027C16.6577 10.5955 16.6575 11.0704 16.3652 11.3633L10.5312 17.1963C10.2383 17.4892 9.76354 17.4892 9.47065 17.1963L3.63764 11.3633C3.34475 11.0704 3.34475 10.5956 3.63764 10.3027C3.93056 10.0102 4.4054 10.0099 4.69819 10.3027L9.25092 14.8555V3.33301C9.2511 2.91905 9.58696 2.58318 10.0009 2.58301Z" fill="#FC7827" />
                                </g>
                                <defs>
                                    <clipPath id="clip0_4580_8402">
                                        <rect width="16" height="16" fill="white" />
                                    </clipPath>
                                </defs>
                            </svg>
                        </button>
                        <!-- TODO: add dropdown  -->
                        <div class="sorting__dropdown" data-id="time">
                            <ul class="sorting__dropdown-list">
                                <li class="sorting__option active" data-value="За все время">За все время</li>
                                <li class="sorting__option" data-value="За 24 часа">За 24 часа</li>
                                <li class="sorting__option" data-value="За неделю">За неделю</li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <ul class="catalog__list offers-grid js-list" data-id="vacancies">
            <? for ($i = 0; $i < 4; $i++) {
                include $_SERVER["DOCUMENT_ROOT"] . '/include/vacancy-item.php';
            } ?>
        </ul>

        <div class="scroll-trigger"></div>

        <? // include $_SERVER["DOCUMENT_ROOT"] . '/include/pagination.php'; ?>
    </div>
</section>

<? require($_SERVER["DOCUMENT_ROOT"] . "/include/searching-save.php"); ?>

<? require($_SERVER["DOCUMENT_ROOT"] . "/include/filter.php"); ?>

<? require($_SERVER["DOCUMENT_ROOT"] . "/template/footer.php"); ?>